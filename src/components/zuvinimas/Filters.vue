...
import { getFishTypes } from '@/utils/requests/zuvinimas';
import moment from 'moment';
import { inject, ref } from 'vue';
const mapLayers: any = inject('mapLayers');

const emit = defineEmits(['change']);
const filters = mapLayers.filters('zuvinimas').onAll();

const startingYearOfFishStockings = 2022;
const years = new Array(moment().get('year') - startingYearOfFishStockings + 1)
  .fill(0)
  .map((_, index) => startingYearOfFishStockings + index)
  .reverse();
...
const selectedFish = ref(`${filters.get('fishType') || ''}` as string);
...
function applyFishFilter() {
  applyFilter(Number(selectedFish.value), 'fishType');
}
